...
import { IDragProps, IDragStatus, TTarget } from "./Draggable.types";



export const Draggable = ({ children, onDrag, getTarget, deps = [] }: IDragProps) => {
    ...
    const handleMouseUp = () => {
        if (!dragRef.current.isOn) { return }
    ...
    const handleMouseMove = (e: MouseEvent) => {
        ...
        if (resp) {
            const { leftWidth = dragRef.current.currPosition, width = dragRef.current.currWidth } = resp;
